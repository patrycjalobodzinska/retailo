...
export default function QASectionV1({ data }: { data?: QAData } = {}) {
  const { t } = useLang();
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // Czasowa animacja po wejściu sekcji w viewport — niezależna od
      // tempa scrolla. Wcześniejsza wersja używała ScrollTrigger pin+scrub
      // i blokowała scrolla na 120vh, co nie pasowało do reszty layoutu.
      tiles.forEach((el, i) => {
        const col = i % 3;
        gsap.set(el, { y: COL_INITIAL_Y[col], opacity: 0 });
      });
